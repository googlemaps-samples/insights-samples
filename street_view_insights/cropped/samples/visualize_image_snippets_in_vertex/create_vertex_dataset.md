...
- `DATASET_ID`: The prefix name of the generated dataset.
...
The output filename will be determined by the fully-qulafieid BigQuery table path.
...
python3 create_vertex_dataset.py \
    ...
    --table_id "full_frame_observations_latest"
```

The script will query BigQuery, locally generate a JSONL file ending with a random 4-letter alphanumeric suffix to guarantee uniqueness (e.g.
`project_id.imagery_insights_sample.full_frame_observations_latest._10_abcd.jsonl`),
...
6. Paste the **GCS URI** that was copied at the end of Step 2 (e.g., `gs://{your_bucket}/misc/${PROJECT_ID}.${DATASET_ID}.${TABLE_ID}_10_abcd.jsonl`) into the *Import file path* box.
7. Click **Continue**.
